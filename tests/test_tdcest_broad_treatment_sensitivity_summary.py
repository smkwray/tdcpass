...
def test_tdcest_broad_treatment_sensitivity_summary_reports_variant_moves() -> None:
    sensitivity = pd.DataFrame(
        [
            ...
            {"treatment_variant": "tier2_regression_mmf_rrp_bank_only", "treatment_family": "measurement", "outcome": "total_deposits_bank_qoq", "horizon": 0, "beta": 0.9, "se": 0.2, "lower95": 0.5, "upper95": 1.3, "n": 40},
            {"treatment_variant": "tier2_regression_mmf_rrp_bank_only", "treatment_family": "measurement", "outcome": "other_component_qoq", "horizon": 0, "beta": -0.35, "se": 0.2, "lower95": -0.75, "upper95": 0.05, "n": 40},
            {"treatment_variant": "tier2_regression_mmf_rrp_di_np_cu", "treatment_family": "measurement", "outcome": "total_deposits_bank_qoq", "horizon": 0, "beta": 0.95, "se": 0.2, "lower95": 0.55, "upper95": 1.35, "n": 40},
            {"treatment_variant": "tier2_regression_mmf_rrp_di_np_cu", "treatment_family": "measurement", "outcome": "other_component_qoq", "horizon": 0, "beta": -0.37, "se": 0.2, "lower95": -0.77, "upper95": 0.03, "n": 40},
    ...
    assert (
        abs(
            h0["variants"]["tier2_regression_mmf_rrp_bank_only"]["total_deposits_delta_vs_baseline"]
            - (-0.1)
        )
        < 1e-12
    )
...
def test_tdcest_broad_treatment_sensitivity_summary_reports_insufficient_history() -> None:
    ...
    assert payload["reason"] == "some_corrected_tdcest_variants_do_not_clear_current_shock_history_gate"
    ...
    assert payload["recommendation"]["status"] == "use_available_long_history_rows_and_keep_short_rows_as_level_comparison"
    ...
    shocked = pd.DataFrame(
        {
            ...
            "tdc_tier2_regression_mmf_rrp_prop_bank_only_qoq": [8.2, 9.2, 7.2, 10.2, 8.2, 11.2, 10.2, 12.2, 9.2, 13.2, 11.2, 14.2, 12.2],
            "tdc_tier2_regression_mmf_rrp_prop_di_np_cu_qoq": [8.3, 9.3, 7.3, 10.3, 8.3, 11.3, 10.3, 12.3, 9.3, 13.3, 11.3, 14.3, 12.3],
