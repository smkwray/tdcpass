...
def test_strict_private_offset_residual_summary_identifies_small_offset_structure() -> None:
    quarter_count = 20
    ...
    households = [0.2 if idx % 2 == 0 else -0.2 for idx in range(quarter_count)]
    ...
    payload = build_strict_private_offset_residual_summary(
        shocked=shocked,
        baseline_lp_spec={
        ...
        strict_nonfinancial_corporate_bridge_summary={"status": "available"},
        horizons=(0,),
    )

    assert payload["status"] == "available"
    assert payload["recommendation"]["status"] == "private_offset_diagnostic_role"
    assert payload["recommendation"]["next_branch"] == "assess_nonfinancial_noncorporate_offset_role"
    h0_core = payload["key_horizons"]["h0"]["core_deposit_proximate"]
    assert h0_core["interpretation"] == "offset_is_nonfinancial_noncorporate_dominant"
    assert h0_core["private_offset_total_response"] is not None
    assert h0_core["private_minus_corporate_response"] is not None
    assert h0_core["offset_alignment_gap_beta"] is not None
    assert payload["takeaways"][0].startswith("This surface isolates the remaining private detail")
