...
def test_strict_nonfinancial_corporate_bridge_summary_isolates_corporate_bridge() -> None:
    quarter_count = 20
    ...
    payload = build_strict_nonfinancial_corporate_bridge_summary(
        shocked=shocked,
        baseline_lp_spec={
        ...
        strict_private_borrower_bridge_summary={"status": "available"},
        horizons=(0,),
    )

    assert payload["status"] == "available"
    assert payload["recommendation"]["status"] == "nonfinancial_corporate_bridge_first"
    assert payload["recommendation"]["next_branch"] == "assess_household_and_nonfinancial_noncorporate_offset_residual"
    h0_core = payload["key_horizons"]["h0"]["core_deposit_proximate"]
    assert h0_core["interpretation"] == "nonfinancial_corporate_explains_private_bridge"
    assert h0_core["nonfinancial_corporate_response"] is not None
    ...
    assert payload["takeaways"][0].startswith("This surface narrows the private-borrower bridge")
